'''
Only for 2D US Needle Segmentation in water environment
'''
...
dir_img = 'C:\Research\projects\Learning\dataset\data_training\Data_Water/imgs'
dir_mask = 'C:\Research\projects\Learning\dataset\data_training\Data_Water/masks'
dir_checkpoint = 'checkpoints/'

####### train parameters #######
val_percent = 0.10  # train dataset size: 1710, val dataset size: 571 test dataset size: 254
epoch_num = 80
batch_size_train = 8
batch_size_val = 1
set_determinism(642) # 设置随机种子!!!!!!!!!!!!!1
#################################
...
def main(tempdir):
    ...
    # debug: 确定是flip存在问题,没有flip的时候loss下降很快
    # 已解决: Transform的问题-img和seg相同的部分必须对齐，尤其是flip！！！否则就会出现img和label不对应的情况，原因可能是其内部同步机制是基于执行顺序的！！！
    train_imtrans = Compose(
        [
            ...
            ScaleIntensity(),  
            RandFlip(prob=0.7), # 两个方向都有可能翻转，会在ArrayDataset里面自动同步 与RandAxisFlip效果没有本质区别
    ...
    train_segtrans = Compose(
        [
            ...
            Resize((512, 512)), 
            ScaleIntensity(),
            RandFlip(prob=0.7), # 顺序必须一致才能对齐img/mask变换
        ]
    )
        ...
        if (epoch + 1) % val_interval == 0: # 每隔val_interval个epoch进行一次validation
            ...
            with torch.no_grad():       
                ...
                if metric > best_metric:
                    ...
                    torch.save(model.state_dict(), "best_model_AttUnet_Water.pth")
